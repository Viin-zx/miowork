...
export const PROGRAMMATIC_EXEC_STDIN_DESCRIPTION =
  'Owned request body for MioWork Programmatic Tool call or batch commands'
const PROGRAMMATIC_EXEC_DESCRIPTION =
  'For MioWork Programmatic Tool commands, use miowork tool search --query "<terms>" [--limit <n>], miowork tool describe --target <name>, call, or batch. Describe targets may use exact double quotes, but prefer the unquoted returned name; never use single quotes or escapes. Pass call and batch JSON through stdin. Omit timeoutMs, background, and yieldMs entirely for Programmatic commands; ordinary shell commands retain these exec controls.'
