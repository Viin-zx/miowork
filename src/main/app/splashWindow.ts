  ...
  <body>
    <div class="shell">
      <form id="panel" class="panel">
        ...
        <p id="hint" class="hint">MioWork is reading the saved password from the system credential store.</p>
      </form>
    </div>
    <script>
      ...
      const setDebugMode = (mode) => {
        ...
        if (mode === 'loading') {
          ...
          hint.textContent = 'MioWork is starting.'
          return
        }
        if (mode === 'system-unlock') {
          ...
          actions.hidden = true
          subtitle.textContent = 'Unlocking local database'
          hint.textContent = 'MioWork is reading the saved password from the system credential store.'
          return
        }
        if (mode === 'recovery') {
          label.hidden = false
