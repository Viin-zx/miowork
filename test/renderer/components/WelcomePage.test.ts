...
describe('WelcomePage', () => {
  it.skipIf(!GUIDED_ONBOARDING_ENABLED)(
    'marks init complete and navigates provider entry to provider settings',
    async () => {
      vi.resetModules()
      vi.useFakeTimers()
  ...
  it.skipIf(!GUIDED_ONBOARDING_ENABLED)(
    'uses the primary onboarding action to resume the first chat step',
    async () => {
      vi.resetModules()
  ...
  it.skipIf(!GUIDED_ONBOARDING_ENABLED)(
    'blocks background clicks and lets the spotlight coachmark continue the real select-provider step',
    async () => {
      vi.resetModules()
